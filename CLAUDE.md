...
- No test suite. Node/npm availability varies by session/environment — check with `node --version` before assuming it's absent; when it's there, `npm install` (no `node_modules`/`package-lock.json` committed — a `.gitignore` was added 2026-08-10 covering `.env`/`node_modules`/`build`, but don't `git add` those anyway if it's ever missing) then `CI=true npm run build` is a real compile check and should be preferred over manual brace/paren balancing. As of 2026-07-30 it was available and `npm run start` (dev server on :3000) also works for a quick "does it render" smoke test.
- **As of 2026-08-10, a local `.env` exists** (`REACT_APP_SUPABASE_URL` + `REACT_APP_SUPABASE_ANON_KEY`, a `sb_publishable_...` key the user provided) — git-ignored, check for it before assuming "no Supabase access." Direct REST reads against the `weekly_report` table (`curl .../rest/v1/weekly_report?report_id=eq.<key>&select=*` with `apikey`/`Authorization: Bearer` headers set to the anon key) are confirmed working and are a fast way to check what's actually persisted without spinning up the dev server. Writes and a logged-in dev-server session haven't been exercised yet — the app's own auth goes through `api/auth.js` (a Vercel serverless function), which a plain `npm start` dev server doesn't serve, so logging in locally likely still needs `vercel dev` or the deployed site.
